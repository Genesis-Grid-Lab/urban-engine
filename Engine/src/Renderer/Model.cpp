...
void Model::LoadModel(const std::string &path) {
  ...
  int joints = m_Skeleton->num_joints();
  for (int i = 0; i < joints; i++) {
    UE_CORE_WARN("[MODEL LOAD] Joints: {}", m_Skeleton->joint_names()[i]);
  }
  UE_CORE_TRACE("MODEL SKEL NUM BONE: {}", joints);
  // m_InverseBindMatrices.resize(joints, ozz::math::Float4x4::identity());
  ...
  for (int i = 0; i < joints; i++) {
    ...
    // // UE_CORE_TRACE("JointName: {}", jointName);
    // // UE_CORE_TRACE("boneinfomap size: {} joint size {}",
    // m_BoneInfoMap.size(),
    ...
    mat.cols[0] = ozz::math::simd_float4::Load(offset[0][0], offset[1][0],
                                               offset[2][0], offset[3][0]);
    mat.cols[1] = ozz::math::simd_float4::Load(offset[0][1], offset[1][1],
                                               offset[2][1], offset[3][1]);
    mat.cols[2] = ozz::math::simd_float4::Load(offset[0][2], offset[1][2],
                                               offset[2][2], offset[3][2]);
    mat.cols[3] = ozz::math::simd_float4::Load(offset[0][3], offset[1][3],
...
Ref<Mesh> Model::ProcessMesh(aiMesh *mesh, const aiScene *scene) {
  ...
  UE_CORE_WARN("Mesh with {0} vertices and {1} indices", vertices.size(),
               indices.size());
