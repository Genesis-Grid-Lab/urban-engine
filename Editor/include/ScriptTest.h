...
public:
  virtual void OnCreate() override {
    ...
    auto view =
        m_Scene->GetRegistry().view<TransformComponent, CameraComponent>();
  ...
  virtual void OnUpdate(Timestep ts) override {
    ...
    // auto &charComp =
    //     GetComponent<CharacterComponent>(); // NEW API: provides WishMove,
    // MaxSpeed, Body, etc.
    auto &tc = GetComponent<TransformComponent>();
    ...
    camData.SetOffset(camDir * 5.0f +
                      glm::vec3(0, 2, 0)); // third-person offset
    ...
    // NEW API: write the desired horizontal velocity; physics system consumes
    // this in its MoveKinematic step
    // charComp.WishMove = moveDir * speed;

    // Jump flag (if you later add vertical handling in your PhysicsUpdate for
    // characters)
    ...
    // UE_INFO("PC WishMove len={:.3f} jump={}", glm::length(charComp.WishMove),
    //         (int)charComp.Jump);
  }
